...
import { getAccessLogById } from 'apiSdk/access-logs';
import { AccessLogInterface } from 'interfaces/access-log';

function AccessLogViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<AccessLogInterface>(
    () => (id ? `/access-logs/${id}` : null),
    () =>
      getAccessLogById(id, {
        relations: [],
      }),
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('access_log', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/access-logs/edit/${id}`} passHref legacyBehavior>
                    <Button
